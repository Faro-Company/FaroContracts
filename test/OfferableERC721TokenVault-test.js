...
describe('OfferableERC721TokenVault', function () {
    ...
    it('Cannot be started by non-owner', async function () {
        const factoryUser = this.ercVaultFactory.connect(this.distributor);
        const offeringAddress = await factoryUser.getVault(0);
        const OfferingVault = await ethers.getContractFactory('OfferableERC721TokenVault');
        const offeringVault = await OfferingVault.attach(offeringAddress);
        const nonOwnerSigner = offeringVault.connect(this.distributor);
        await expectRevert(nonOwnerSigner.start());
    });
    ...
    it('Can be started by the owner', async function() {
        ...
        const offeringVault = await OfferingVault.attach(offeringAddress);
        const ownerSigner = offeringVault.connect(this.owner);
        const startTx = await ownerSigner.start();
        await startTx.wait();
        expect(await ownerSigner.getOfferingState()).to.equal(1);
    })
    ...
    it('Cannot bid if not among funders', async function() {
        ...
        const offeringVault = await OfferingVault.attach(offeringAddress);
        const nonOwnerSigner = offeringVault.connect(this.distributor);
        await expectRevert(nonOwnerSigner.bid(100, {value: 1000}));
    });
